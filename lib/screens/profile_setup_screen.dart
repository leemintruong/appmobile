...
class _ProfileSetupScreenState extends State<ProfileSetupScreen> {
  ...
  num get _calorieGoal {
    switch (_goal) {
      case 'lose_weight':
        return (_tdee - 500).clamp(1200, 5000);
      case 'gain_weight':
        return _tdee + 300;
      case 'build_muscle':
        return _tdee + 200;
  ...
  Future<void> _save({bool finish = false}) async {
    ...
    try {
      ...
      final goal = await ApiService.setGoal(
        _goal,
      ...
      if (profile['success'] == false || goal['success'] == false) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              profile['message'] ?? goal['message'] ?? 'Không lưu được hồ sơ',
            ),
          ),
        );
        return;
              ...
              children: [
                ...
                Expanded(
                  child: SkOutlineButton(
                    ...
                    onPressed: _saving ? null : () => _save(finish: true),
                  ),
                ),
              ],
            ),
      ...
      child: Column(
        ...
        children: [
          ...
          _chips(
            'Mức hoạt động',
            {
              'sedentary': 'Dân văn phòng',
              'moderate': 'Lao động nặng',
              'active': 'Chuyên nghiệp',
            },
            _activity,
    ...
    return SkCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            '${_calorieGoal.toStringAsFixed(0)} kcal / ngày',
            style: const TextStyle(
              color: Colors.white,
              fontSize: 30,
              fontWeight: FontWeight.w900,
  ...
  String _goalHint() {
    switch (_goal) {
      case 'lose_weight':
        return 'Ứng dụng đề xuất giảm nhẹ calo và ưu tiên protein để giảm cân an toàn.';
      case 'gain_weight':
        return 'Ứng dụng đề xuất tăng nhẹ calo để hỗ trợ tăng cân lành mạnh.';
      case 'build_muscle':
        return 'Tăng cơ cần đủ calo và protein, kết hợp vận động đều.';
      default:
        return 'Mức calo này giúp bạn duy trì cân nặng ổn định.';
    }
  }
}
